...
const { getStorage } = require("firebase-admin/storage");
const Message = require("../models/Message.js");
const Group = require("../models/Group.js"); // グループモデルをインポート
const mongoose = require("mongoose");
const { getIo } = require("../socket/index.js");
...
router.post("/", upload.single("file"), async (req, res) => {
  try {
    ...
    await message.save(); // 新しいメッセージが届いたことをSocket.IOで通知

    const io = getIo(); // グループのメンバーに通知を送信（送信者を除く）
    const groupData = await Group.findById(group);
    if (groupData) {
      groupData.members.forEach((memberId) => {
        if (memberId.toString() !== sender) {
          io.to(memberId.toString()).emit("message_received", {
            groupId: group,
          });
        }
      });
